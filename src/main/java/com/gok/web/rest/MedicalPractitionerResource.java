...
import com.gok.service.dto.PersonDTO;
...
public class MedicalPractitionerResource {
    ...
    private static final String ENTITY_NAME = "medicalpractitionerMedicalPractitioner";
     ...
     *
     * @param medicalPractitioner the medicalPractitionerDTO to create.
    ...
    public ResponseEntity<List<MedicalPractitionerDTO>> createMedicalPractitioner(@RequestBody List<MedicalPractitioner> medicalPractitioner) throws URISyntaxException {
        log.debug("REST request to save MedicalPractitioner : {}", medicalPractitioner);
        medicalPractitioner.forEach(medicalPractitioner1 -> {
            if (medicalPractitioner1.getId() != null) {
                throw new BadRequestAlertException("A new medicalPractitioner cannot already have an ID", ENTITY_NAME, "idexists");
            }
            PersonDTO personDTO = new RestTemplate().getForObject("http://localhost:8422/people/"+medicalPractitioner1.getPersonIdRef(), PersonDTO.class);
             if(personDTO.getId() == null)
                 throw new BadRequestAlertException("No person exsists with this person id", ENTITY_NAME, "idna");
            ...
            List<MedicalPractitionerDTO> result = medicalPractitionerService.save(medicalPractitioner);
            return ResponseEntity.created(new URI("/api/medical-practitioners/"))
    ...
    */
     ...
     *
     * @param medicalPractitioner the medicalPractitionerDTO to update.
    ...
    public ResponseEntity<List<MedicalPractitionerDTO>> updateMedicalPractitioner(@RequestBody List<MedicalPractitioner> medicalPractitioner) throws URISyntaxException {
        log.debug("REST request to save MedicalPractitioner : {}", medicalPractitioner);
        medicalPractitioner.forEach(medicalPractitioner1 -> {
            if (medicalPractitioner1.getId() != null) {
                throw new BadRequestAlertException("A new medicalPractitioner cannot already have an ID", ENTITY_NAME, "idexists");
